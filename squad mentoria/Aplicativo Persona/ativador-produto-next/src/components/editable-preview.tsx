...
import { X, Play, ExternalLink, ShoppingCart } from "lucide-react"
...
export function EditablePreview({
  ...
  const handleConfirm = () => {
    if (!d.ctaLink || d.ctaLink === "#") {
      const confirmed = window.confirm("Nenhum link de compra definido. O botão CTA não terá destino. Continuar?")
      if (!confirmed) return
    }
        ...
        <div className="p-4 sm:p-6 space-y-5">
          ...
          <div className="flex gap-3 pt-2 sticky bottom-0 bg-[#F5EFE8] pb-2">
            ...
            >
              ...
              Confirmar & Publicar Página de Vendas
            </Button>
            <Button variant="outline" onClick={onCancel} className="px-6 border-[#D9CEC2] text-[#5C5146]">
              Cancelar
            </Button>
          </div>
        </div>
